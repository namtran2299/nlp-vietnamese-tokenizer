...
namespace std {
    ...
    Machine::Machine(int window_length, string path, StrMapReference ref) {
        ...
        _model = new model;
        memset(_model,0,sizeof(model));

        if (ref == LEARN){
            vector<featuresOfSyllabel>* vfeats = extractTest("một ví_dụ", ref); // dont erase it.
            delete vfeats;
        }
    }
    ...
    vector<featuresOfSyllabel>* Machine::convert(string sentence, Feats* feats) {
...
//        while (sentence[length - 1] <= 40 && length > 0) sentence.erase(--length);
    ...
    vector<featuresOfSyllabel>* Machine::extractTest(string sentence, StrMapReference ref) {
        Feats feats;
        return extract(sentence, ref, &feats);
    }

    /* Convert a string to vfeats, extract features and put it in feats */
    vector<featuresOfSyllabel>* Machine::extract(string sentence, StrMapReference ref, Feats* feats) {
        vector<featuresOfSyllabel>* vfeats;
        // convert sentence(string) to vfeats;
        vfeats = convert(sentence, feats);
    ...
    problem Machine::getProblem(Feats* feats) {
        problem _problem;
        memset(&_problem,0,sizeof(problem));
    ...
    void Machine::delProblem(problem& _problem) {
    ...
    problem Machine::training(Feats* feats) {
        // init parameter
        ...
        problem _problem = getProblem(feats);
        if (_model) {
            free_and_destroy_model(&_model);
        }
    ...
    /* do close test */
    double Machine::close_test(Feats* feats, problem& _problem) {
        int count = 0;
    ...
    bool Machine::load() {
        ...
        _model = load_model( std::string(PATH + "dongdu.model").c_str() );
        if (!strmap.load(PATH + "dongdu.map")) {
    ...
    string Machine::segment(string sentence) {
        ...
        vector<featuresOfSyllabel>* vfeats = extract(sentence, PREDICT, feats);
        if (feats->size() == 0) {
            delete feats;
            delete vfeats;
            printf("No items!\n");
