...
int learner_main(int argc, char **argv)
{
	...
	while (ifs && getline(ifs, sentence)) {
		vector<featuresOfSyllabel>* vfeats = learner.extract(sentence, LEARN,feats);
                delete vfeats;
		line_num ++;
		if (line_num % 10000 == 0)
			cout << line_num << endl;
	...
	problem   _problem = learner.training(feats);
	learner.close_test(feats,_problem);
	learner.print();
	ifs.close();
        delete feats;
        learner.delProblem(_problem);
