...
string Feats::type(string word)
{
	...
	for(int i = 0; i < int(word.length()); ++i){
		if (int(SYMBOLS.find(word[i])) > -1) { kh = true; continue;}
		if (word[i] >= '0' && word[i] <= '9') {cs = true;continue;}
		if (word[i] >= 'A' && word[i] <= 'Z') { VH = true;continue;}
		if (word[i] >= 'a' && word[i] <= 'z') {vt = true;continue;}
	}
